...
export function Catalog() {
  ...
  return <><PageHeading eyebrow="Возможности для совместной работы" title="Каталог задач" description="Реальные потребности бизнеса. Выберите задачу, в которой ваша команда может быть полезна." action={actor?.role === 'business' && <Link className="button" to="/business/tasks/new"><Plus size={17} />Создать задачу</Link>} />
    <div className="catalog-banner"><div className="catalog-banner-copy"><span className="feature-label"><ArrowDownWideNarrow size={13} />ОТ ПОТРЕБНОСТИ К ПРОЕКТУ</span><h2>Найдите свой<br /><span>следующий вызов.</span></h2><p>Больше ясности — выше в каталоге.<br />Откликнуться можно на задачу любого уровня.</p></div><StudioArtwork /><span className="banner-note">0–100 <small>баллов готовности</small></span></div>
